    ...
    <v-app-bar color="primary" app clipped-left dark>
      ...
      <v-menu offset-y open-on-hover>
        <template v-slot:activator="{ on, attrs }">
          <v-avatar class="elevation-3" v-on="on" v-bind="attrs">
            <v-img
              :src="
          ...
          <v-card-text class="pa-0">
            <table width="400">
              ...
              <tr v-for="item in Object.keys(userInfo)" :key="item">
                ...
                <td class="pr-2" v-html="item.toString().toLowerCase()=='correo'?`<a href=mailto:'${userInfo[item]}'>${userInfo[item]}</a>`:userInfo[item]"></td>
              </tr>
            </table>
          </v-card-text>
          <v-card-actions>
            <v-spacer></v-spacer>
            <v-btn color="primary" @click="msgbox('Hola')">Cambiar imagen</v-btn>
            <v-btn color="warning" @click="logout()">Cerrar sesión</v-btn>
            
          </v-card-actions>
        </v-card>
      </v-menu>
      <v-btn fab small dark color="error" @click="salir()" class="ml-1"><v-icon>mdi-exit-run</v-icon> </v-btn>
    </v-app-bar>
...
export default {
  ...
  methods: {
    ...
    login: function (handler) {
      let mv = this;
      let remoteUrl = "http://coopefacsa.coop:3100";
    ...
    openChat:function(mensaje){
      let mv = this;
      let user ={}
      if(mensaje.userType==0){
        user=mv.$store.state.usuarios.filter(u=>{return u.id=mensaje.to})[0]
        user.userType=0
      }else{
        user=mv.$store.state.salas.filter(s=>{return s.id==mensaje.to})[0]
      }
      mv.setTo(user)
    }
  },
  ...
  mounted: function () {
    ...
    mv.login((res) => {
      ...
      if(!browserId){
        browserId = (new Date().toLocaleString()+Math.random(100)).replace(/[/ ,:.]/g,"")
        localStorage.setItem("browserId",browserId)
      }
      let socketIo=io(mv.$store.state.server,{
        query:`userId=${user.id}&userName=${user.nombre}&browserId=${browserId}`
      });
      socketIo.on("mensaje",data=>{
        console.log(data)
        mv.$store.commit("addMessage",data)
        let grupo  = data.userType==0?'usuarios':'salas'
        let userIdx = mv.$store.state[grupo].findIndex(u=>{return data.from==u.id})
        let notifyOptions = {
          title:data.userType==0?mv.$store.state[grupo][userIdx].nombre:`Sala ${mv.$store.state[grupo][userIdx].nombre}`,
          body:data.texto
        }
        ipcRenderer.send("mensaje",notifyOptions)
        ipcRenderer.on("openChat",(event,args)=>{
          console.log(event)
          mv.openChat(args)
        })
      })
